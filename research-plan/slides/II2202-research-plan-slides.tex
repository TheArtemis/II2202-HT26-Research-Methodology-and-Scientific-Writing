...
% ------------------------------------------------------------
\begin{frame}[t]{Research topic and problem}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.36\textwidth}
      {\footnotesize
      \textbf{Problem.}\\[0.2em]
      Raft needs a majority quorum.
      Under some partial partitions that quorum cannot be kept.
      Overlay routing is proposed, but little tested with Raft.

      \vspace{0.55em}
      \textbf{RQ1.}\\[0.15em]
      In which partitions does forwarding restore stable progress?

      \vspace{0.45em}
      \textbf{RQ2.}\\[0.15em]
      How does added hop delay change commit latency?
      }
    \end{column}
    \begin{column}{0.62\textwidth}
      \centering
      \includegraphics[height=6.05cm,keepaspectratio]{../figures/topology-scenarios.png}\\[0.15em]
      {\scriptsize\color{quiet}Planned topologies T0--T4 (direct vs.\ transparent forwarding)}
    \end{column}
  \end{columns}
\end{frame}
